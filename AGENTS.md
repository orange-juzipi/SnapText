...
- `python3 scripts/build_frontend.py`：生成 `ui/pkg`；需要 `wasm-bindgen`。
...
不要提交 API Key、签名凭据、真实用户历史数据库或大型模型二进制。发布证据文件应基于 `docs/` 中的示例维护。发布前检查生成产物，例如 `dist/`、`target/` 和 `ui/pkg`。
